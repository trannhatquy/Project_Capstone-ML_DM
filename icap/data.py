...
class Vocabulary(object):
    def __init__(self, freq_threshold, spacy_eng=None):
        ...
        self.freq_threshold = freq_threshold  # frequency threshold
        if spacy_eng == None:
            self.spacy_eng = spacy.load("en_core_web_sm")  # spacy english model
        else:
            self.spacy_eng = spacy_eng  # spacy english model

    def __len__(self):
        return len(self.itos)  # len(vocab)

    def tokenizer_eng(self, text):
        tokenizer = [
            tok.text.lower() for tok in self.spacy_eng.tokenizer(text)
        ]  # tokenize text
        return tokenizer

    def build_vocabulary(self, sentence_list):  # build vocabulary
        frequencies = {}
        idx = 4
    ...
    def numericalize(self, text):  # numericalize text
...
class FlickrDataset(Dataset):
    def __init__(
        ...
        self.dataframe = pd.read_csv(caption_file, delimiter=caption_delimiter) # read caption.txt
        self.transform = transform # transform image

        self.vocab = Vocabulary(freq_threshold) 
        self.vocab.build_vocabulary(self.dataframe[self.text_column].tolist()) # build vocabulary

        self.train = train # train or valid
        self.split_val = split_val # ratio of train and valid
    ...
    def _do_split_train_valid(self):
        (
            ...
            self.dataframe[self.image_column],
